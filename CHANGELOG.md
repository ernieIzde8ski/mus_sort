...
- fix `setuptools` missing from `requirements-dev.txt`
- removed `black` from `requirements-dev.txt`
- add `.pre-commit-config.yaml`, `.editorconfig` files for devs
  - update formatting with `pre-commit`
- update tooling in `pyproject.toml`
